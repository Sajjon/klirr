...
/// Result type for calendar-related computations.
pub type CalendarResult<T, E = CalendarError> = std::result::Result<T, E>;

/// Error type for shared calendar computations.
...
fn period_end_for_unit(date: Date, unit: Granularity) -> CalendarResult<Date> {
    match unit {
        ...
        unsupported => Err(CalendarError::InvalidPeriod {
            bad_value: format!(
...
fn period_end_for_cadence(date: Date, cadence: Cadence) -> CalendarResult<Date> {
...
/// ```
pub fn normalize_period_end_date_for_cadence(date: Date, cadence: Cadence) -> CalendarResult<Date> {
...
fn fortnight_serial(period_end: Date) -> CalendarResult<i32> {
...
fn shift_period_end(period_end: Date, unit: Granularity, amount: i16) -> CalendarResult<Date> {
...
fn elapsed_periods_since(start: Date, end: Date, cadence: Cadence) -> CalendarResult<u16> {
    let start = period_end_for_cadence(start, cadence)?;
    let end = period_end_for_cadence(end, cadence)?;
    if start > end {
        return Err(CalendarError::StartPeriodAfterEndPeriod {
...
/// ```
pub fn period_end_from_relative_time(relative: RelativeTime) -> CalendarResult<Date> {
...
fn parse_legacy_period_label(value: &str) -> CalendarResult<(Date, Granularity)> {
...
/// ```
pub fn parse_period_label_for_cadence(value: &str, cadence: Cadence) -> CalendarResult<Date> {
    if let Ok((date, unit)) = parse_legacy_period_label(value) {
        return match (cadence, unit) {
            (Cadence::Monthly, Granularity::Fortnight) => {
                Err(CalendarError::CannotExpenseForFortnightWhenCadenceIsMonthly)
            }
            (Cadence::BiWeekly, Granularity::Month) => {
                Err(CalendarError::CannotExpenseForMonthWhenCadenceIsBiWeekly)
            }
...
/// Calculates the period number from an offset and target period-end date.
///
/// This is the shared, invoice-agnostic version of invoice-number stepping logic.
///
/// # Examples
/// ```
/// extern crate klirr_foundation;
...
/// use klirr_foundation::*;
///
/// let offset_number = 100u16;
/// let offset_date = "2024-01-31".parse::<Date>().unwrap();
...
/// .collect::<IndexSet<_>>();
///
/// let number = calculate_period_number(
///     offset_number,
///     &offset_date,
...
pub fn calculate_period_number(
    ...
    let offset_date = period_end_for_cadence(*offset_date, cadence)?;
    if record_of_periods_off.contains(&offset_date) {
        return Err(CalendarError::RecordsOffMustNotContainOffsetPeriod {
    ...
    for period_off in record_of_periods_off {
    ...
    let mut number = offset_number + periods_elapsed_since_offset - periods_off_to_subtract;
    if is_expenses {
        number += 1;
    }
...
fn period_bounds(period_end: Date, cadence: Cadence) -> CalendarResult<(Date, Date)> {
...
pub fn quantity_in_period(
    ...
    if record_of_periods_off.contains(&target_date) {
        return Err(CalendarError::TargetPeriodMustNotBeInRecordOfPeriodsOff {
    ...
    if matches!(
        ...
        return Err(CalendarError::CannotInvoiceForMonthWhenCadenceIsBiWeekly);
    }
...
mod tests {
    use super::*;
    use crate::{HasSample, ModelError};
    use rust_decimal::dec;
    ...
    fn periods_off(dates: impl IntoIterator<Item = Date>) -> IndexSet<Date> {
    ...
    fn period_end_for_unit_rejects_unsupported_granularity() {
        ...
        assert!(matches!(result, Err(CalendarError::InvalidPeriod { .. })));
    }
    ...
    fn elapsed_periods_since_fails_when_start_after_end() {
        ...
        assert_eq!(
            result.unwrap_err(),
            CalendarError::StartPeriodAfterEndPeriod {
    ...
    fn parse_label_rejects_invalid_shape_or_mismatched_kind() {
        ...
        assert!(matches!(
            missing_month,
            Err(CalendarError::Model(ModelError::FailedToParseDate { .. }))
        ));
    }
    ...
    #[test]
    fn calculate_period_number_monthly() {
        let offset_date = d("2025-12-31");
        let target = d("2026-02-28");
        let number = calculate_period_number(
            100,
            &offset_date,
            &target,
    ...
    #[test]
    fn calculate_period_number_subtracts_periods_off_and_adds_expenses_increment() {
        let offset_date = d("2024-01-31");
        let target = d("2024-04-30");
        let periods_off = periods_off([d("2024-02-29")]);
    ...
    #[test]
    fn calculate_period_number_rejects_offset_in_periods_off() {
        let offset_date = d("2025-12-31");
        let periods_off = periods_off([d("2025-12-31")]);
        let result = calculate_period_number(
            100,
            &offset_date,
            &d("2026-01-31"),
            Cadence::Monthly,
        ...
        assert_eq!(
            result.unwrap_err(),
            CalendarError::RecordsOffMustNotContainOffsetPeriod {
    ...
    #[test]
    fn calculate_period_number_fails_when_target_is_before_offset() {
        let offset_date = d("2026-02-28");
        let result = calculate_period_number(
            100,
            &offset_date,
            &d("2026-01-31"),
