...
pub fn version(base_path: impl AsRef<Path>) -> Result<Version> {
    ...
    if !path.exists() && base_path.exists() {
        // Don't auto-migrate: we don't know the actual version, so we can't
        // safely assume the current version. Return an error requiring explicit migration.
        return Err(Error::data_version_mismatch(Version::V0, Version::current()));
    }
...
mod tests {
    ...
    #[test]
    fn read_data_returns_data_version_mismatch_when_version_file_is_missing() {
        let tempdir = tempfile::tempdir().expect("Failed to create temp dir");
        save_data_with_base_path(Data::sample(), tempdir.path()).unwrap();
