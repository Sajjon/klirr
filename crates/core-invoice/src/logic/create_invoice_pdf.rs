...
pub fn create_invoice_pdf_with_data_base_path(
    data_base_path: impl AsRef<Path>,
    input: ValidInput,
    render: impl Fn(L10n, PreparedData, Layout) -> Result<Pdf>,
) -> Result<NamedPdf> {
...
pub fn create_invoice_pdf_with_data<Period: IsPeriod>(
    data: Data<Period>,
    input: ValidInput,
    render: impl Fn(L10n, PreparedData, Layout) -> Result<Pdf>,
) -> Result<NamedPdf> {
    let l10n: L10n = get_localization(input.language())?;
...
mod tests {
    ...
    fn test_create_pdf() {
        ...
        let named_pdf =
            create_invoice_pdf_with_data::<YearAndMonth>(Data::sample(), input, |_, _, _| {
